...
def smoothing_filter(img_path= Path):
    ...
    #kernerl_size = M_values                             # M values
    ...
    for version in range(1,5):
        noise_img_name = str(img_path.name)[:str(img_path.name).find("_img")]+"_0"+str(version)
        path_img_noise= Path(img_path.parents[1],"imgs_noise",noise_img_name+".png")
        ...
        for Msize in M_values:
                ...
                for j in range(k, n-k):
                    for l in range (0,kernerl_size):
                        for c in range(0,kernerl_size):
                            temp[l,c] = img_original[i-k+l, j-k+c]
            ...
            cv2.imwrite(str(Path(img_path.parents[1],"imgs_smooth",filter_img_name)), img_mean)
            cv2.imwrite(str(Path(img_path.parents[1],"imgs_smooth",filter_img_name)), img_median)

            print("M = "+str(Msize)+", PSNRs: average="+str(calculate_erro(img_original, img_mean)) + ", median=" + str(calculate_erro(img_original, img_median)))
